...
class BookCallbackFactory(CallbackData, prefix="books"):
    action: str
    id: Optional[int] = None


class BookSettingsCallbackFactory(CallbackData, prefix="settingsbook"):
    action: str
...
def main_books_kb() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text='Добавить книгу', callback_data=BookCallbackFactory(action='add_book'))
    builder.button(text='Найти книгу', callback_data=BookSettingsCallbackFactory(action='search_book'))
    builder.button(text='Поиск по QR-коду', callback_data=BookCallbackFactory(action='qr_search'))
...
def back_to_search_settings_from_book_info(book):
    builder = InlineKeyboardBuilder()
    builder.button(text='отмена', callback_data=BookSettingsCallbackFactory(action='search_book'))
    builder.button(text='добавить экземпляры', callback_data=BookCallbackFactory(action='add_book_instance', id=book.id))
    builder.adjust(1)
    return builder.as_markup()


def back_to_search_settings_from_instance_add():
    builder = InlineKeyboardBuilder()
    builder.button(text='отмена', callback_data=BookSettingsCallbackFactory(action='search_book'))
    builder.adjust(1)
    return builder.as_markup()
...
def book_qr_search_menu(book) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text='Возврат книги', callback_data=BookCallbackFactory(action='back_to_library', id=book.id))
    builder.button(text='🔙 Назад', callback_data='books_main')
    return builder.as_markup()