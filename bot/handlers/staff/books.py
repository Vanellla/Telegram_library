...
from bot.keyboards.staff.books_main_keyboards import main_books_kb, BookCallbackFactory, parallel_choice_kb, \
    genre_choice_kb, book_cancel_kb, author_choice_kb, book_back_to_main_kb, book_search_settings_kb, \
    BookSettingsCallbackFactory, genre_config_kb, parallel_config_kb, back_to_search_settings, author_config_kb, \
    BookSearchCallbackFactory, book_search_result_kb, back_to_search_settings_from_book_info, \
    back_to_search_settings_from_instance_add, book_qr_search_menu
from bot.states.staff_states import Content_add
from db import Parallel, Klass, Genre, Author, Book_info, Book
from docs import make_word_table_with_qr
from qr import DecodeQR
from settings import tmp_files, qr_dir

books_main_router = Router()


@books_main_router.callback_query(F.data == 'books_main')
async def books_main(clb: types.CallbackQuery, state: FSMContext, bot: Bot) -> None:
    await state.clear()
...
async def callbacks_book_search_result_fab(callback: types.CallbackQuery, callback_data: BookCallbackFactory,
    ...
    text += f'\nЖанр: {book.genre.name}'
    text += f'\nПараллель: {book.parallel.name}'
    book_instances = book.count()
    text += f'\nЭкземпляров в наличии: {book_instances}'
    await callback.message.edit_text(text=text, reply_markup=back_to_search_settings_from_book_info(book))


# result_set = set()


@books_main_router.callback_query(BookCallbackFactory.filter(F.action == 'add_book_instance'))
async def callbacks_book_add_instance(callback: types.CallbackQuery, callback_data: BookCallbackFactory,
                                      state: FSMContext):
    book_id = callback_data.id
    book = Book_info.get(id=book_id)
    text = f'Название: {book.name}.\nВведите количество экземпляров, которое хотите добавить в базу.'
    await state.set_state(Content_add.book_instance_count)
    await state.update_data(msg_id=callback.message.message_id)
    await state.update_data(book_info_id=book.id)
    await callback.message.edit_text(text=text, reply_markup=back_to_search_settings_from_instance_add())


@books_main_router.message(Content_add.book_instance_count)
async def book_add_instance(msg: types.Message, state: FSMContext, bot: Bot):
    instance_count = msg.text
    contex_data = await state.get_data()
    msg_id = contex_data.get('msg_id')
    book_info_id = contex_data.get('book_info_id')
    book_info = Book_info.get(id=book_info_id)
    book_id_list = []
    for _ in range(int(instance_count)):
        book = Book.create(book=book_info)
        book_id_list.append(book.id)
    doc = make_word_table_with_qr(book_id_list, book_info.name)
    await msg.answer_document(document=FSInputFile(doc), caption='Файл с QR-кодами для вклейки в книгу')
    text = f'Информация о книге:'
    text += f'\nНазвание: {book_info.name}'
    text += f'\nАвтор: {book_info.author.fio}'
    text += f'\nЖанр: {book_info.genre.name}'
    text += f'\nПараллель: {book_info.parallel.name}'
    book_instances = book_info.count()
    text += f'\nЭкземпляров в наличии: {book_instances}'
    await msg.answer(text=text, reply_markup=back_to_search_settings_from_book_info(book_info))
...
@books_main_router.callback_query(BookCallbackFactory.filter(F.action == "qr_search"))
async def callbacks_book_qr_search(callback: types.CallbackQuery, callback_data: BookCallbackFactory,
                                   state: FSMContext):
    await state.set_state(Content_add.qr_search)
    await state.update_data(msg_id=callback.message.message_id)
    text = 'Пришлите фото QR-кода, чтобы начать поиск.'
    await callback.message.edit_text(text=text, reply_markup=book_cancel_kb())


@books_main_router.message(Content_add.qr_search)
async def book_qr_search(msg: types.Message, state: FSMContext, bot: Bot):
    ...
    if code != '':
        ...
        book_instances = book_info.count()
        text += f'\nЭкземпляров в наличии: {book_instances}'
        if book.books.count() > 0 and book.books[-1].is_return == False:
            last = book.books[-1]
            text += f'\nВыдана: {last.student.fio} {last.student.klass.name}'
            await msg.answer(text=text, reply_markup=book_qr_search_menu(book))
        else:
            text += f'\nНикому не выдана'
            await msg.answer(text=text, reply_markup=book_cancel_kb())
    else:
        text = '❌ Мне не удалось распознать QR-код'
        await msg.answer(text=text, reply_markup=book_cancel_kb())


@books_main_router.callback_query(BookCallbackFactory.filter(F.action == "back_to_library"))
async def callbacks_book_back_to_library(callback: types.CallbackQuery, callback_data: BookCallbackFactory,
                                         state: FSMContext):
    book_id = callback_data.id
    ...
    await callback.message.edit_text(text=text, reply_markup=book_back_to_main_kb())
